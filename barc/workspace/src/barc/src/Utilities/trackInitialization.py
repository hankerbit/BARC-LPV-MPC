...
class Map():
    ...
    def __init__(self, flagTrackShape = 0):
        ...
        if selectedTrack == "3110":
            self.halfWidth = 0.4
            self.slack     = 0.15
            spec = np.array([[60 * 0.03, 0, v_high],
                             [80 * 0.03, +80 * 0.03 * 2 / np.pi, v_high],
                             [20 * 0.03, 0, v_high],
                             [80 * 0.03, +80 * 0.03 * 2 / np.pi, v_high],
                             [40 * 0.03, -40 * 0.03 * 10 / np.pi, v_high],
                             [60 * 0.03, +60 * 0.03 * 5 / np.pi, v_high],
                             [40 * 0.03, -40 * 0.03 * 10 / np.pi, v_high],
                             [80 * 0.03, +80 * 0.03 * 2 / np.pi, v_high],
                             [20 * 0.03, 0, v_high],                             
                             [80 * 0.03, +80 * 0.03 * 2 / np.pi, v_high],
            ...
            self.halfWidth = 0.5 
            self.slack     = 0.15
            spec = np.array([[1.0, 0, v_high],
                             [4.5, 4.5 / np.pi, v_high],
                             [2.0, 0, v_high],
                             [4.5, 4.5 / np.pi, v_high],
                             [1.0, 0, v_high]])


        # EA: Modified for introducing also the desired velocity (third column)
        elif selectedTrack == "Euge_Track":
            self.halfWidth = 0.4
            self.slack     = 0.15
            spec = np.array([[30 * 0.03, +30 * 0.03 * 2 / np.pi, v_med],
                             [20 * 0.03, -0, v_high],
                             [30 * 0.03, -30 * 0.03 * 2 / np.pi, v_high],
                             [30 * 0.03, +30 * 0.03 * 2 / np.pi, v_high],
                             [30 * 0.03, +30 * 0.03 * 2 / np.pi, v_high],
                             [130 * 0.03, 0 , v_soH],
                             [30 * 0.03, +30 * 0.03 * 2 / np.pi, v_high],
        ...
        # PointAndTangent = [x, y, psi, cumulative s, segment length, signed curvature, liner velocity]
        PointAndTangent = np.zeros((spec.shape[0] + 1, 7))
        for i in range(0, spec.shape[0]):
            if spec[i, 1] == 0.0:              # If the current segment is a straight line
                ...
                if i == 0:
                    NewLine = np.array([x, y, psi, PointAndTangent[i, 3], l, 0, spec[i,2]])
                else:
                    NewLine = np.array([x, y, psi, PointAndTangent[i-1, 3] + PointAndTangent[i-1, 4], l, 0, spec[i,2]])
                ...
                if i == 0:
                    NewLine = np.array([x, y, psi, PointAndTangent[i, 3], l, 1 / r, spec[i,2]])
                else:
                    NewLine = np.array([x, y, psi, PointAndTangent[i-1, 3] + PointAndTangent[i-1, 4], l, 1 / r, spec[i,2]])
        ...
        NewLine = np.array([xf, yf, psif, PointAndTangent[-2, 3] + PointAndTangent[-2, 4], l, 0, 0.8])
        PointAndTangent[-1, :] = NewLine
